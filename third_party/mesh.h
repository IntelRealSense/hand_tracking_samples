...
std::vector<int3> adjacencies(const std::vector<int3> &tris)
{
	...
	v1tab.resize(vcount);
	for (int i = (int)edges.size(); i--;) // by going in backwards order we end up with v1tab indexing back so that 
...
inline Mesh MeshSmoothish(const std::vector<float3> &points, const std::vector<int3> tris,bool justnormals=false)
{
	std::vector<Vertex> verts;
	for (auto p : points)
	...
	for (auto t : tris)
	{
		...
		auto q = quatfrommat(float3x3(st, sb, sn)); 
		for (int i = 0; i < 3; i++)
		{
			verts[t[i]].orientation += (justnormals) ? float4{sn, 0} : q;
		}
	}
	for (auto &v : verts)
	{
		v.orientation = (justnormals) ? quat_from_to(float3(0, 0, 1), normalize(v.orientation.xyz()))   : normalize(v.orientation);
...
inline Mesh MeshTexSphereMap(Mesh mesh)  // might need to call this with std::move() to avoid copying
{
	auto vsqrt = [](float3 v) { return float3{ sqrt(v.x),sqrt(v.y),sqrt(v.z) };};
	auto positions = Transform(mesh.verts, [](Vertex v) {return v.position;});
	auto pca = PrincipalAxes(positions);
	pca.first.orientation = qmul(pca.first.orientation,QuatFromAxisAngle(normalize(float3(1, 0, 1)), 3.14f)); // // let z be the long, not the shortest eigen  so 180 rot on x+z
	std::swap(pca.second.z, pca.second.x);
	auto spheremap = [](float3 n) {
		n = normalize(n);
		return float2{ atan2(n.x, n.y) / (3.14f*2.0f),atan2(n.z,length(n.xy())) / 3.14f + 0.5f };
	};
	for (auto &v : mesh.verts)
		v.texcoord = spheremap(((pca.first.inverse()*v.position) /*  /vsqrt(pca.second)*/  ));    // non oriented alternative is to just use (v.position-pca.first.position)   ignoring eigenvalues to better make use of tex real estate
	return mesh;
}

inline Mesh MeshAppend(Mesh m0, const Mesh &m1)
{
	for (auto &t : m1.tris)
		m0.tris.push_back(t + int3((int)m0.verts.size()));
	Append(m0.verts, m1.verts);
	return m0;
}
