...
template<class T> std::vector<T*>  Addresses(      std::vector<T> &a) { std::vector<T*> b; b.resize(a.size()); for(unsigned int i=0; i < a.size(); ++i) {b[i] = &a[i];} return b; }
template<class T> std::vector<T*>  Addresses(const std::vector<T> &a) { std::vector<T*> b; b.resize(a.size()); for(unsigned int i=0; i < a.size(); ++i) {b[i] = &a[i];} return b; }
