...
float3  physics_gravity(0, 0, -10.0f);
...
float   physics_damping = 0.15f;  // 1 means completely damped,  0 means no damping

int     physics_iterations = 16;
int     physics_iterations_post = 4;
int     physics_use_collision = 1;
...
inline float3 CenterOfMass(const std::vector<Shape> &meshes)
{
	...
	for (auto &m : meshes)
	{
		...
		float3 cg = CenterOfMass(verts, tris.data(), (int)tris.size());
		float   v = Volume(verts, tris.data(), (int)tris.size());
		vol += v;
		com += cg*v;
	}
	com /= vol;
...
inline float3x3 Inertia(const std::vector<Shape> &meshes, const float3& com)
{
	...
	for (auto &m : meshes)
	{
		...
		const float3 *verts = m.verts.data();
		float v  =  Volume (verts, tris.data(), (int)tris.size() );
		inertia +=  Inertia(verts, tris.data(), (int)tris.size(), com) * v;
		vol += v;
	}
	inertia *= 1.0f / vol;
	return inertia;
...
class RigidBody : public State 
{
  public:
	...
	Pose            PoseUser() const { return   Pose{ PositionUser(),orientation }; }
...
inline std::vector<LimitLinear> CollisionConstraints(const std::vector<RigidBody*> &rigidbodies, const std::vector<std::vector<float3> *> &wgeom)
{
...
inline void PhysicsUpdate(const std::vector<RigidBody*> &rigidbodies, std::vector<LimitLinear> &Linears, std::vector<LimitAngular> &Angulars, const std::vector<std::vector<float3> *> &wgeom)
{

	for (auto &rb : rigidbodies)
		rbinitvelocity(rb);          // based on previous and current force/torque
	if (physics_use_collision) {
