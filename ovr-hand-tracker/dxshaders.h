...
float4 texonly( Fragment v) : SV_Target
{
	return txDiffuse.Sample( samLinear,v.texcoord);
}
float4 texlit( Fragment v) : SV_Target
{
	float d =  dot(normalize(v.normal)  ,normalize(float3(-1,-2,1)) ) ;
	return (0.5+ 0.5*clamp(d,0,1 )) * txDiffuse.Sample( samLinear,v.texcoord);
}

float4 rimlit( Fragment v) : SV_Target
{
	float3 vdir = normalize(camerap - v.position);
	float intensity = 1.0f - pow(dot(v.normal,vdir),0.5) * 0.75;
	//return float4(1,0,0,1);
	return float4(hack.xyz*intensity,1.0);
}
