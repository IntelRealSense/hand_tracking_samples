...
struct WingMesh
{
	...
	int BuildEdge(int ea, int eb)  // puts an edge from ea.v to eb.v  and splits a face into two
	{
		...
		int newface = (int)faces.size();
		WingMesh::HalfEdge sa((int)edges.size() + 0, edges[ea].v, (int)edges.size() + 1, eb, edges[ea].prev, newface);  // id,v,adj,next,prev,face
		WingMesh::HalfEdge sb((int)edges.size() + 1, edges[eb].v, (int)edges.size() + 0, ea, edges[eb].prev, edges[ea].face);
	...
	void SplitEdge(int e, const float3 &vpos)
	{
		...
		WingMesh::HalfEdge s0((int)edges.size() + 0, v, (int)edges.size() + 1, edges[e].next, e, edges[e].face);  // id,v,adj,next,prev,face
		WingMesh::HalfEdge sa((int)edges.size() + 1, edges[ea].v, (int)edges.size() + 0, ea, edges[ea].prev, edges[ea].face);
	...
	std::vector<int> SliceMesh(const float4 &slice)  // also creates new edges along the split plane
	{
		...
		if (it == verts.end())
			return loop;
		int v0 = (int)(it - verts.begin()), v = v0;
	...
	std::vector<std::vector<int>> ExportPolygons() const
	{
		std::vector<std::vector<int>> polygons;
		for (int i = 0; i < (int)faces.size(); i++)
		{
			std::vector<int> poly;
			for (auto e : FaceView(i))
				poly.push_back(e.v);
	...
	float  CalcVolume() const { auto tris = GenerateTris(); return Volume(verts.data(), tris.data(), (int)tris.size()); }

};

inline float3    SupportPoint(const WingMesh *m, const float3& dir) { return m->verts[maxdir(m->verts.data(), (int)m->verts.size(), dir)]; }
...
inline WingMesh & WingMeshSort(WingMesh &m) // not sure if this routine works now
{
	...
	for(unsigned int i=0;i<m.faces.size();i++)
	{
        ...
        do{ s.push_back(m.edges[e]); m.edges[e].id = (int)s.size() - 1; e = m.edges[e].next; } while (e != m.fback[i]);
	}
...
inline WingMesh WingMeshSubDiv(WingMesh wm)
{
	...
	for (int fid = 0;fid < nf;fid++)
	{
		...
		wm.SplitEdge((int)wm.edges.size() - 1, fvertsnew[fid]);
		int eid_new= (int)wm.edges.size() - 2; // this whould be the one with the correct face
		assert(wm.edges[eid_new].v == wm.verts.size() - 1);
		assert(wm.edges[eid_new].face == fid);
		for (int e = (int)eids.size() - 1; e >= 2;e--)
...
inline WingMesh WingMeshCreate(std::vector<float3> verts, std::vector<std::vector<int>> faces)
{
	WingMesh wm;
	wm.verts = std::move(verts);
	for (auto &vids : faces)
	{
		int base = (int)wm.edges.size();
		std::vector<float3> fverts;
		for (int i = 0; i < (int)vids.size(); i++)
		{
			int inext = (i + 1) % vids.size();
			int iprev = (i + (int)vids.size() - 1) % (int)vids.size();
			wm.edges.push_back(WingMesh::HalfEdge(base + i, vids[i], -1, base + inext, base + iprev, (int)wm.faces.size()));
...
// treat halfedges as an array of ints, i.e. just a datablock, to avoid export spewage
// template<class F> void visit_fields(WingMesh &o, F f){ f("verts", o.verts); f("faces", o.faces); f("vback", o.vback); f("fback", o.fback); f("edges", *((std::vector<int[sizeof(WingMesh::HalfEdge) / sizeof(int)]> *) &o.edges));}

template<class F> void visit_fields(WingMesh &o, F f)   // rebuild version for minimal data storage
{ 
	std::vector<std::vector<int>> faces = o.ExportPolygons();
	std::vector<float3> verts = std::move(o.verts);
	f("verts", verts);
	f("faces", faces);
	o = WingMeshCreate(verts, faces);
}
...
{
    std::vector<float3> verts;
    int faceId = (int)mesh.faces.size(), baseEdge = (int)mesh.edges.size();
    for(int i=0; i<n; ++i)
    {
...
inline WingMesh WingMeshCylinder(int sides, float radius, float height)
{
    ...
    WingMeshAddFace(mesh, bottom.data(), (int)bottom.size());
    WingMeshAddFace(mesh, top.data()   , (int)top.size()   );
